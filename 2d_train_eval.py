...
def main(data_flag, output_root, num_epochs, gpu_ids, batch_size, download, model_flag, resize, as_rgb, model_path, run):
    ...
    train_data = DataClass(split='train', transform=data_transform, download=download, as_rgb=as_rgb)
    val_data = DataClass(split='val', transform=data_transform, download=download, as_rgb=as_rgb)
    test_data = DataClass(split='test', transform=data_transform, download=download, as_rgb=as_rgb)

    train_loader = data.DataLoader(dataset=train_data, batch_size=batch_size, shuffle=True)
    train_loader_eval = data.DataLoader(dataset=train_data, batch_size=batch_size, shuffle=False)
    ...
    if model_flag == 'resnet18':
        model = resnet18(pretrained=False, num_classes=n_classes) if \
            resize else ct2dModel.ResNet18(in_channels=n_channels, num_classes=n_classes)
    elif model_flag == 'resnet50':
        model = resnet50(pretrained=False, num_classes = n_classes) if \
            resize else ct2dModel.ResNet50(in_channels=n_channels, num_classes=n_classes)
    else:
    ...
    if model_path is not None:
        ...
        train_metrics = test(model, train_evaluator, train_loader_eval, task, criterion, device, run, output_root)
    ...
    writer = SummaryWriter(log_dir = 'D:\pytorch\logs')
    ...
    for epoch in trange(num_epochs):
        ...
        train_metrics = test(model, train_evaluator, train_loader_eval, task, criterion, device, run)
    ...
    path = os.path.join(output_root, 'best_ct_2d_model.pat')
    torch.save(state, path)

    train_metrics = test(best_model, train_evaluator, train_loader_eval, task, criterion, device, run, output_root)
